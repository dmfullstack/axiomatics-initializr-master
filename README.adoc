= Axiomatics Spring Initializr image:https://badges.gitter.im/spring-io/initializr.svg[link="https://gitter.im/spring-io/initializr?utm_source=badge&utm_medium=badge&utm_campaign=pr-badge&utm_content=badge"]
...
Axiomatics Spring Initializr is a fork of Spring Initialzr that includes Axiomatics PEP dependencies. This is a personal project and not officially supported by Axiomatics.

IMPORTANT: Both Gradle and Maven builds are supported. Groovy and Kotlin are not supported yet. Many other features are planned too and some can be seen in the issues page here on Github.
...
The groupIds and artificatIds used by the Axiomatics Spring Initializr are the following:
